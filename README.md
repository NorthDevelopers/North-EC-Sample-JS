...
2. **`.env`** lives in the **project root** (same folder as this README). Create it if missing, or edit the one you already have. Set at least `API_URL`, `PRIVATE_API_KEY`, and `HOST` (comma-separated hostnames, e.g. `www.test.com,test.com`). Optional: `CHECKOUT_ID`, `PROFILE_ID` (defaults exist in code).
